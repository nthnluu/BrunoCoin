...
func (n *Node) ChkBlk(b *block.Block) bool {
	...
	if !b.SatisfiesPOW(n.Mnr.DifTrg()) {
		return false
	}
	...
	for txIndex := range b.Transactions {
		...
		if txn.IsCoinbase() && txIndex != 0 {
			return false
		}

		if len(txn.Inputs) < 1 && !txn.IsCoinbase() {
			return false
		}
...
func (n *Node) ChkTx(t *tx.Transaction) bool {
	...
	if len(t.Inputs) < 1 && !t.IsCoinbase() {
		return false
	}
	...
	for txInputIndex := range t.Inputs {
		txInput := t.Inputs[txInputIndex]
		if txInput == nil {
			return false
		}
		...
		if !n.Chain.GetUTXO(txInput).IsUnlckd(txInput.UnlockingScript) {
			return false
		}
	}
