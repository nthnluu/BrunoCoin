...
func (m *Miner) GenCBTx(txs []*tx.Transaction) *tx.Transaction {
	...
	for txIndex := range txs {
		var transaction = txs[txIndex]
		txFee = txFee + (transaction.SumInputs() - transaction.SumOutputs())
	}
	...
	// and gets cut in half every c.SubsdyHlvRt until
	// c.MxHlvgs
	numTimesToHalf := m.ChnLen.Load() / m.Conf.SubsdyHlvRt

	if numTimesToHalf > m.Conf.MxHlvgs {
		numTimesToHalf = m.Conf.MxHlvgs
	}

	subsdy := float64(m.Conf.InitSubsdy) * math.Pow(0.5, float64(numTimesToHalf))

	// 3. Then a transaction is made with zero inputs
	// and with an output paying the miner fees + mint
	var newOutput = proto.NewTxOutpt(txFee + uint32(subsdy), hex.EncodeToString(m.Id.GetPublicKeyBytes()))
	var newTx = proto.NewTx(m.Conf.Ver, []*proto.TransactionInput{}, []*proto.TransactionOutput{newOutput}, m.Conf.DefLckTm)

	return tx.Deserialize(newTx)
}
