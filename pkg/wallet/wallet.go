...
func (w *Wallet) HndlBlk(b *block.Block) {
	...
	// be incrementing the priorities, removing duplicates,
	// and returning which ones are "old"
	removedPriority, _ := w.LmnlTxs.ChkTxs(b.Transactions)


	// 2. Resend out "old" liminal transactions with the
	// lock time incremented (in order to give the transaction
	// a different hash since lock time essentially does nothing)
	var toResend []*tx.Transaction
	for removedIndex := range removedPriority {
		removedTx := removedPriority[removedIndex]
		removedTx.LockTime = removedTx.LockTime + 1
		toResend = append(toResend, removedTx)
	}

	for txIndex := range toResend {
		w.LmnlTxs.Add(toResend[txIndex])
		w.SendTx <- toResend[txIndex]
	}
...
func (w *Wallet) HndlTxReq(txR *TxReq) {
	...
	totalToSpend := txR.Amt + txR.Fee

	// 1. Try and find enough UTXO to make the transaction
	utxos, change, isEnoughUTXO := w.Chain.GetUTXOForAmt(totalToSpend, hex.EncodeToString(w.Id.GetPublicKeyBytes()))
	...
	// 3. Make the transaction inputs for the transaction
	// from the UTXO
	var txInputs []*proto.TransactionInput = []*proto.TransactionInput{}
	for utxoIndex := range utxos {
		utxo := utxos[utxoIndex]
		newSig, sigError := utxo.UTXO.MkSig(w.Id)
		if sigError != nil {
			return
		}
		newTxi := proto.NewTxInpt(utxo.TxHsh, 0, newSig, utxo.Amt)
		txInputs = append(txInputs, newTxi)
	}

	// 4. Make the transaction outputs based on who you
	// send money to and if there is change leftover for
	// yourself
	var txOutputs []*proto.TransactionOutput = []*proto.TransactionOutput{}
	newTxo := proto.NewTxOutpt(txR.Amt, hex.EncodeToString(txR.PubK))
	txOutputs = append(txOutputs, newTxo)
	if change > 0 {
		changeTxo := proto.NewTxOutpt(change, hex.EncodeToString(w.Id.GetPublicKeyBytes()))
		txOutputs = append(txOutputs, changeTxo)
	}

	newTx := proto.NewTx(w.Conf.TxVer, txInputs, txOutputs, w.Conf.DefLckTm)
	dTx := tx.Deserialize(newTx)
