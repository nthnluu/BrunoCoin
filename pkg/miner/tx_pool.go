...
func (tp *TxPool) Add(t *tx.Transaction) {
	...
	if tp.Ct.Load() < tp.Cap {
		...
		// the correct priority
		newPri := CalcPri(t)
		tp.CurPri.Store(tp.CurPri.Load() + newPri)
		tp.Ct.Inc()
		tp.TxQ.Add(newPri, t)
	}
...
func (tp *TxPool) ChkTxs(remover []*tx.Transaction) {
	...
	for elemI := range remover {
		if remover[elemI] == nil {
			return
		}
	}

	// Lock the transaction pool
	tp.mutex.Lock()
	defer tp.mutex.Unlock()
	...
	// Update count and total priority fields
	var pri uint32 = 0
	for i := range remover {
		pri = pri + CalcPri(remover[i])
	}

	// Update priority and count
	tp.CurPri.Store(tp.CurPri.Load() - pri)
	tp.Ct.Sub(uint32(len(remover)))
