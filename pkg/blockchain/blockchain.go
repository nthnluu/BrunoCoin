...
func (bc *Blockchain) Add(b *block.Block) {
	...
	// 1. Find previous node (that this block is being appended
	// to)
	var prevNode = bc.LastBlock

	// 2. From the previous node's utxo, remove any used utxo
	// and add the new utxo from the new block

	// init new utxo as prev block's utxo
	var newUTXO = prevNode.utxo

	// iterate over transactions
	for txIndex := range b.Transactions {
		var transaction = b.Transactions[txIndex]
		for txiIndex := range transaction.Inputs {
			// iterate over transaction inputs
			var txInput = transaction.Inputs[txiIndex]
			txKey := txo.MkTXOLoc(txInput.TransactionHash, txInput.OutputIndex)
			delete(newUTXO, txKey)
			println(len(newUTXO))
		}

		for txoIndex := range transaction.Outputs {
			txOutput := transaction.Outputs[txoIndex]
			txKey := txo.MkTXOLoc(transaction.Hash(), uint32(txoIndex))
			newUTXO[txKey] = txOutput
		}
	}
	...
	println("Block added to the blockchain! " + "Block " + b.NameTag())
	return
}
...
func (bc *Blockchain) GetUTXOForAmt(amt uint32, pubKey string) ([]*UTXOInfo, uint32, bool) {
	...
	var txoInfos []*UTXOInfo
	var accumAmt uint32 = 0

	// 1. Find the utxo on the last block of the main chain.
	var utxo = prevBlock.utxo
	for txHash, transactionOutput := range utxo {
		if (transactionOutput.LockingScript == pubKey) && (accumAmt < amt) {
			// add amt to accumulated amount
			accumAmt = accumAmt + transactionOutput.Amount

			// generate txo info
			utxoHash, utxoIndex := txo.PrsTXOLoc(txHash)
			var newTxoInfo = &UTXOInfo{utxoHash, utxoIndex, transactionOutput, transactionOutput.Amount}
			txoInfos = append(txoInfos, newTxoInfo)
		}
	}

	isEnough := false
	var remainingAmt uint32 = 0

	if accumAmt >= amt {
		isEnough = true
		remainingAmt = accumAmt - amt
	}

	return txoInfos, remainingAmt, isEnough
}
